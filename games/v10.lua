...
function BlessedHubX:Window(GuiConfig)
    ...
    if GuiConfig.AnimColor then
        ...
        accentColorFunc = function()
            local h, s, v2 = Color3.toHSV(c)
            return Color3.fromHSV((h + tick() * 0.05) % 1, s, v2)
        end
    end
    ...
    LayersTab.BackgroundTransparency = TRANS + 0.2
    ...
    DropdownSelect.BackgroundTransparency = TRANS
    ...
    function Tabs:AddTab(TabConfig)
        ...
        function TabItems:AddDropdown(cfg2)
            ...
            -- Selector pill animated border — uses cfg2.AnimColor or default
            local selColorFunc = nil
            if cfg2.AnimColor then
                local c = cfg2.AnimColor
                selColorFunc = function()
                    local h, s, v2 = Color3.toHSV(c)
                    return Color3.fromHSV((h + tick() * 0.08) % 1, s, v2)
                end
            end
        ...
        function TabItems:AddDiscord(DiscordConfig)
            ...
            MakeAnimatedGradient(cardStroke, card, function()
                return Color3.fromHSV((0.67 + math.sin(tick() * 1.2) * 0.05) % 1, 0.6, 1)
            end)
            ...
            JoinBtn.Size             = UDim2.new(0, isMobile and 90 or 80, 0, isMobile and 32 or 28)
            ...
            if not isMobile then
                JoinBtn.MouseEnter:Connect(function()
                    TweenService:Create(JoinBtn, TweenInfo.new(0.15), {BackgroundColor3=Color3.fromRGB(110,120,255)}):Play()
                end)
...
--[[
═══════════════════════════════════════════════════════
  EXAMPLE USAGE  —  copy & adapt for your script
═══════════════════════════════════════════════════════

local Library = loadstring(game:HttpGet("YOUR_RAW_URL_HERE"))()

-- ──────────────────────────────────────────────────
--  Window setup  (all fields are optional)
-- ──────────────────────────────────────────────────
local Window = Library:Window({
    Title      = "My Hub",          -- Window title
    Footer     = "v1.0",            -- Subtitle next to title
    Color      = Color3.fromRGB(255, 105, 180),  -- Accent / theme color
    Background = Color3.fromRGB(14, 14, 20),     -- Window background color
    Transparency = 0.3,             -- 0 = opaque, 1 = invisible (global)
    Size       = UDim2.new(0, 640, 0, 420),      -- Window size (width, height)
    AnimColor  = nil,               -- nil = rainbow border; Color3 = fixed hue
    Version    = 1,                 -- Config file version (bump to reset saved configs)
})

-- ──────────────────────────────────────────────────
--  Tabs
-- ──────────────────────────────────────────────────
local Tab1 = Window:AddTab({ Name = "Aimbot",  Icon = "crosshair" })
local Tab2 = Window:AddTab({ Name = "Visual",  Icon = "eyes" })
local Tab3 = Window:AddTab({ Name = "Discord", Icon = "discord" })
local Tab4 = Window:AddTab({ Name = "Settings",Icon = "settings" })

-- ──────────────────────────────────────────────────
--  Aimbot Tab
-- ──────────────────────────────────────────────────
Tab1:AddTextSeparator("AIMBOT")

local aimbotToggle = Tab1:AddToggle({
    Title    = "Enable Aimbot",
    Default  = false,
...
local smoothSlider = Tab1:AddSlider({
    Title     = "Smooth",
    Min       = 1,
    Max       = 100,
    Default   = 20,
...
local fovSlider = Tab1:AddSlider({
    Title     = "FOV",
    Min       = 10,
    Max       = 500,
    Default   = 120,
...
Tab1:AddTextSeparator("HITBOX")

local boneDropdown = Tab1:AddDropdown({
    Title    = "Bone Target",
    Options  = {"Head", "HumanoidRootPart", "UpperTorso", "LowerTorso"},
    Default  = "Head",
    AnimColor = Color3.fromRGB(255, 80, 80),  -- red border animation
    Callback = function(val)
        print("Bone:", val)
    end,
})
...
local multiDropdown = Tab1:AddDropdown({
    ...
    Callback = function(vals)
        print("Selected:", table.concat(vals, ", "))
    end,
})
...
-- ──────────────────────────────────────────────────
--  Discord Tab  (simplified — no API calls)
-- ──────────────────────────────────────────────────
local dc = Tab3:AddDiscord({
    Title      = "My Community",
    InviteCode = "yourinvite",   -- code from discord.gg/XXXX
    -- ServerIcon = "rbxassetid://YOUR_ICON_ID",  -- optional custom icon
})

-- You can update fields after creation:
-- dc:SetInvite("newcode")
-- dc:SetTitle("New Title")
-- dc:SetIcon("rbxassetid://123456")

-- ──────────────────────────────────────────────────
--  Notifications
-- ──────────────────────────────────────────────────
-- Quick notification
blessedhubx("Script loaded!", 4, Color3.fromRGB(100,220,100), "Success", "Ready")

-- With custom animation color
blessedhubx("Watch out!", 4, Color3.fromRGB(255,100,100), "Warning", "Check your settings",
    Color3.fromRGB(255, 60, 60))  -- red border animation
...
Library:MakeNotify({
    ...
    Delay       = 6,
    AnimColor   = Color3.fromRGB(255, 200, 50),  -- golden border
})

-- ──────────────────────────────────────────────────
--  Config helpers
-- ──────────────────────────────────────────────────
SaveConfig()           -- manually save current values
LoadConfigElements()   -- restore saved values to all elements
]]
