...
if not isfolder("Blessed Hub X") then
    makefolder("Blessed Hub X")
end
if not isfolder("Blessed Hub X/Config") then
    makefolder("Blessed Hub X/Config")
end
...
local CurrentWindowSize = isMobile and UDim2.new(0, 470, 0, 270) or UDim2.new(0, 640, 0, 400)

-- ─────────────────────────────────────────────
-- Shared animated gradient helper (throttled)
-- ─────────────────────────────────────────────
local function MakeAnimatedGradient(stroke, anchorFrame)
    ...
    conn = RunService.Heartbeat:Connect(function(dt)
        ...
        acc = acc + dt
        if acc < 0.04 then return end   -- ~25 fps
        acc = 0
        rot = (rot + dt * 72) % 360
        local hue  = 0.85 + math.sin(tick() * 1.8) * 0.025
        local col  = Color3.fromHSV(hue, 1, 1)
        Gradient.Color    = ColorSequence.new{ ColorSequenceKeypoint.new(0, col), ColorSequenceKeypoint.new(1, col) }
...
-- Resize
-- ─────────────────────────────────────────────
local function MakeResizable(object, minSizeX, minSizeY)
    minSizeX = minSizeX or 400
    minSizeY = minSizeY or 300
    ...
    inputChangedConn = UserInputService.InputChanged:Connect(function(input)
        if input == resizeInput and resizing then updateSize(input) end
    end)
...
-- Notify
-- ─────────────────────────────────────────────
function BlessedHubX:MakeNotify(NotifyConfig)
    local NotifyConfig = NotifyConfig or {}
    NotifyConfig.Title       = NotifyConfig.Title       or "Blessed Hub X"
    ...
    task.spawn(function()
        ...
        NotifyFrameReal.BackgroundColor3  = Color3.fromRGB(22, 22, 22)
        NotifyFrameReal.BorderSizePixel   = 0
        NotifyFrameReal.Position          = UDim2.new(0, 420, 0, 0)
        NotifyFrameReal.Size              = UDim2.new(1, 0, 1, 0)
        NotifyFrameReal.Name              = "NotifyFrameReal"
        NotifyFrameReal.Parent            = NotifyFrame

        local Stroke = Instance.new("UIStroke")
        Stroke.Parent    = NotifyFrameReal
        Stroke.Thickness = 1.8
        Stroke.Color     = Color3.fromRGB(255, 255, 255)

        MakeAnimatedGradient(Stroke, NotifyFrame)
        ...
        TextLabel1.Position             = UDim2.new(0, TextLabel.TextBounds.X + 20, 0, 0)
        ...
        TextLabel2.Position             = UDim2.new(0, 15, 0, 32)
        TextLabel2.Size                 = UDim2.new(1, -30, 0, 13)
        TextLabel2.TextWrapped          = true
        TextLabel2.Parent               = NotifyFrameReal

        TextLabel2.Size = UDim2.new(1, -30, 0, 13 + (13 * (TextLabel2.TextBounds.X // math.max(TextLabel2.AbsoluteSize.X, 1))))

        if TextLabel2.AbsoluteSize.Y < 27 then
            NotifyFrame.Size = UDim2.new(1, 0, 0, 65)
        else
            NotifyFrame.Size = UDim2.new(1, 0, 0, TextLabel2.AbsoluteSize.Y + 45)
        end
        ...
        function NotifyFunction:Close()
            ...
            TweenService:Create(NotifyFrameReal,
                ...
                { Position = UDim2.new(0, 420, 0, 0) }):Play()
        ...
        TweenService:Create(NotifyFrameReal,
            TweenInfo.new(tonumber(NotifyConfig.Time) * 1.2, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
...
function BlessedHubX:Window(GuiConfig)
    ...
    GuiConfig["Tab Width"] = GuiConfig["Tab Width"] or 70
    ...
    local BlessedHubX     = Instance.new("ScreenGui")
    ...
    BlessedHubX.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    BlessedHubX.Name           = "BlessedHubX"
    BlessedHubX.ResetOnSpawn   = false
    BlessedHubX.Parent         = game:GetService("CoreGui")
    ...
    DropShadowHolder.Parent                 = BlessedHubX
    ...
    -- Animated border on main window
    local Stroke = Instance.new("UIStroke")
    Stroke.Parent    = Main
    Stroke.Thickness = 1.8
    Stroke.Color     = Color3.fromRGB(255, 255, 255)
    MakeAnimatedGradient(Stroke, BlessedHubX)
    ...
    TextLabel1.Position             = UDim2.new(0, TextLabel.TextBounds.X + 55, 0, 0)
    TextLabel1.Parent               = Top
    ...
    MinImg.Size               = UDim2.new(1, -8, 1, -8)
    MinImg.Parent             = Min

    -- Hover animations for Close / Min
    Close.MouseEnter:Connect(function()
        TweenService:Create(CloseImg, TweenInfo.new(0.15), { ImageColor3 = Color3.fromRGB(255, 80, 80) }):Play()
    end)
    Close.MouseLeave:Connect(function()
        TweenService:Create(CloseImg, TweenInfo.new(0.15), { ImageColor3 = Color3.fromRGB(255, 255, 255) }):Play()
    end)
    Min.MouseEnter:Connect(function()
        TweenService:Create(MinImg, TweenInfo.new(0.15), { ImageTransparency = 0 }):Play()
    end)
    Min.MouseLeave:Connect(function()
        TweenService:Create(MinImg, TweenInfo.new(0.15), { ImageTransparency = 0.2 }):Play()
    end)

    -- Tab sidebar
    LayersTab.BackgroundColor3  = Color3.fromRGB(25, 25, 25)
    LayersTab.BorderSizePixel   = 0
    LayersTab.Position          = UDim2.new(0, 10, 0, 45)
    LayersTab.Size              = UDim2.new(0, GuiConfig["Tab Width"], 1, -55)
    ...
    DecideFrame.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
    ...
    -- FIX: content area shifted right so the tab sidebar doesn't overlap
    local TAB_W   = GuiConfig["Tab Width"]
    local CONTENT_X = TAB_W + 24   -- extra 8 px breathing room vs original +20

    Layers.BackgroundTransparency = 1
    Layers.Position               = UDim2.new(0, CONTENT_X, 0, 45)
    Layers.Size                   = UDim2.new(1, -(CONTENT_X + 14), 1, -55)
    Layers.Name                   = "Layers"
    Layers.Parent                 = Main
    ...
    LayersReal.Size                 = UDim2.new(1, 0, 1, -35)
    ...
    LayersPageLayout.TweenTime      = 0.28
    ...
    UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        ScrollTab.CanvasSize = UDim2.new(0, 0, 0, UIListLayout.AbsoluteContentSize.Y + 20)
    end)
    ...
    UserInputService.InputBegan:Connect(function(input, gpe)
        ...
        if input.KeyCode == ToggleKey and DropShadowHolder then ToggleWindow() end
    end)
    ...
    function GuiFunc:ToggleUI()
        ...
        Button.MouseButton1Click:Connect(function()
            if DropShadowHolder then ToggleWindow() end
        end)
        ...
        MainButton.Destroying:Connect(function()
            if inputChangedConn then inputChangedConn:Disconnect() end
        end)
    end
    GuiFunc:ToggleUI()
    ...
    MakeResizable(DropShadowHolder, 400, 300)
    ...
    Close.Activated:Connect(function()
        ...
        Message.Text              = "You won't be able to open it again without executing."
        Message.TextSize          = 13
        ...
        Yes.Size            = UDim2.new(0.45, 0, 0, 35)
        ...
        Cancel.Size            = UDim2.new(0.45, 0, 0, 35)
        Cancel.Position        = UDim2.new(0.5, 0, 1, -45)
        ...
        Yes.MouseButton1Click:Connect(function()
            TweenService:Create(DropShadowHolder, TweenInfo.new(0.35, Enum.EasingStyle.Quint, Enum.EasingDirection.In),
                { Size = UDim2.new(0, 0, 0, 0) }):Play()
            task.wait(0.35)
            if BlessedHubX then BlessedHubX:Destroy() end
    ...
    DropdownSelect.BackgroundColor3 = Color3.fromRGB(28, 28, 28)
    DropdownSelect.Position         = UDim2.new(1, 220, 0.5, 0)
    DropdownSelect.Size             = UDim2.new(0, 200, 1, -20)
    DropdownSelect.ZIndex           = 20
    DropdownSelect.Parent           = MoreBlur
    Instance.new("UICorner", DropdownSelect).CornerRadius = UDim.new(0, 8)

    -- Animated gradient border on the side-panel
    ...
    ConnectButton.Activated:Connect(function()
        if MoreBlur.Visible then
            TweenService:Create(DropdownSelect,
                ...
                { Position = UDim2.new(1, 220, 0.5, 0) }):Play()
            task.wait(0.22)
            MoreBlur.Visible = false
        end
    end)
    ...
    function Tabs:AddTab(TabConfig)
        ...
        UIListLayout1:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            ScrolLayers.CanvasSize = UDim2.new(0, 0, 0, UIListLayout1.AbsoluteContentSize.Y + 20)
        end)
        ...
        FeatureImg.Position               = UDim2.new(0.5, -12, 0.5, -12)
        FeatureImg.Size                   = UDim2.new(0, 24, 0, 24)
        FeatureImg.Image                  = (Icons[TabConfig.Icon] and Icons[TabConfig.Icon] or TabConfig.Icon)
        FeatureImg.Parent                 = Tab

        ChooseFrame.BackgroundColor3     = GuiConfig.Color
        ChooseFrame.Position             = UDim2.new(0, 0, 0.5, -10)
        ChooseFrame.Size                 = UDim2.new(0, 3, 0, 20)
        ChooseFrame.BackgroundTransparency = (CountTab == 0) and 0 or 1
        ...
        -- Tab icon scale pulse on select
            ...
            if Tab.LayoutOrder ~= LayersPageLayout.CurrentPage.LayoutOrder then
                for _, TabFrame in pairs(ScrollTab:GetChildren()) do
                    if TabFrame:IsA("Frame") then
                        TweenService:Create(TabFrame, TweenInfo.new(0.22, Enum.EasingStyle.Quint), { BackgroundTransparency = 1 }):Play()
                        if TabFrame:FindFirstChild("Frame") then
                            TweenService:Create(TabFrame.Frame, TweenInfo.new(0.22, Enum.EasingStyle.Quint), { BackgroundTransparency = 1 }):Play()
                        end
                    end
                end
                TweenService:Create(Tab, TweenInfo.new(0.3, Enum.EasingStyle.Quint), { BackgroundTransparency = 0.5 }):Play()
                TweenService:Create(ChooseFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quint), { BackgroundTransparency = 0 }):Play()
                -- Brief icon pop
                TweenService:Create(FeatureImg, TweenInfo.new(0.12, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
                    { Size = UDim2.new(0, 28, 0, 28), Position = UDim2.new(0.5, -14, 0.5, -14) }):Play()
                task.delay(0.12, function()
                    TweenService:Create(FeatureImg, TweenInfo.new(0.1), { Size = UDim2.new(0, 24, 0, 24), Position = UDim2.new(0.5, -12, 0.5, -12) }):Play()
                end)
        ...
        -- ── Toggle ─────────────────────────────
        function TabItems:AddToggle(ToggleConfig)
            ...
            ToggleTitle.Position         = UDim2.new(0, 15, 0.5, -8)
            ...
            FeatureFrame.Parent           = Card
            Instance.new("UICorner", FeatureFrame).CornerRadius = UDim.new(0, 10)
            ...
            ToggleButton.MouseEnter:Connect(function()
                TweenService:Create(Card:FindFirstChildWhichIsA("UIStroke"), TweenInfo.new(0.18),
                    { Color = Color3.fromRGB(70, 70, 70) }):Play()
            end)
            ToggleButton.MouseLeave:Connect(function()
                if not ToggleFunc.Value then
                    TweenService:Create(Card:FindFirstChildWhichIsA("UIStroke"), TweenInfo.new(0.18),
                        { Color = Color3.fromRGB(45, 45, 45) }):Play()
                end
            end)
            ...
            function ToggleFunc:Set(Value, skipCallback)
                ...
                if Value then
                    TweenService:Create(ToggleTitle,   ti, { TextColor3 = GuiConfig.Color }):Play()
                    TweenService:Create(ToggleCircle,  ti, { Position = UDim2.new(1, -18, 0.5, 0), BackgroundColor3 = Color3.fromRGB(255,255,255) }):Play()
                    TweenService:Create(FeatureFrame,  ti, { BackgroundColor3 = GuiConfig.Color }):Play()
                    TweenService:Create(Card:FindFirstChildWhichIsA("UIStroke"), ti, { Color = GuiConfig.Color }):Play()
                else
                    TweenService:Create(ToggleTitle,   ti, { TextColor3 = Color3.fromRGB(230, 230, 230) }):Play()
                    TweenService:Create(ToggleCircle,  ti, { Position = UDim2.new(0, 2, 0.5, 0) }):Play()
                    TweenService:Create(FeatureFrame,  ti, { BackgroundColor3 = Color3.fromRGB(40, 40, 40) }):Play()
                    TweenService:Create(Card:FindFirstChildWhichIsA("UIStroke"), ti, { Color = Color3.fromRGB(45, 45, 45) }):Play()
                end
            end
        ...
        -- ── Slider ─────────────────────────────
        function TabItems:AddSlider(SliderConfig)
            ...
            ValueBox.TextColor3       = GuiConfig.Color
            ...
            SliderBg.InputBegan:Connect(function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    Dragging = true
                    TweenService:Create(Thumb, TweenInfo.new(0.12), { Size = UDim2.new(0, 13, 0, 13) }):Play()
                    UpdateSlide(Input)
                end
            end)
            SliderBg.InputEnded:Connect(function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    Dragging = false
                    TweenService:Create(Thumb, TweenInfo.new(0.12), { Size = UDim2.new(0, 10, 0, 10) }):Play()
                end
            end)
            ...
            Card.AncestryChanged:Connect(function()
                if not Card.Parent then sliderInputConn:Disconnect() end
            end)

            ValueBox.FocusLost:Connect(function()
                local n = tonumber(ValueBox.Text)
                SliderFunc:Set(n or SliderConfig.Min)
            end)
        ...
        -- ── Button ─────────────────────────────
        function TabItems:AddButton(ButtonConfig)
            local Card = CreateCard()
            Card.Size = UDim2.new(1, -4, 0, 46)

            local MainButton = Instance.new("TextButton")
            MainButton.Font             = Enum.Font.GothamBold
            MainButton.Text             = ButtonConfig.Title or "Button"
            MainButton.TextSize         = 13
            MainButton.TextColor3       = Color3.fromRGB(255, 255, 255)
            MainButton.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
            MainButton.Size             = UDim2.new(1, -30, 1, -16)
            MainButton.Position         = UDim2.new(0, 15, 0, 8)
            ...
            MainButton.MouseEnter:Connect(function()
                TweenService:Create(MainButton, TweenInfo.new(0.15), { BackgroundColor3 = GuiConfig.Color }):Play()
            end)
            MainButton.MouseLeave:Connect(function()
                TweenService:Create(MainButton, TweenInfo.new(0.15), { BackgroundColor3 = Color3.fromRGB(40, 40, 40) }):Play()
            end)
            return Card
        end

        -- ── Dropdown ───────────────────────────
        function TabItems:AddDropdown(DropdownConfig)
            ...
            Dropdown.Size                 = UDim2.new(1, -4, 0, 46)
            Dropdown.Name                 = "Dropdown"
            Dropdown.Parent               = ScrolLayers

            -- Animated gradient border on Dropdown card (active indicator)
            local DropCardStroke = Instance.new("UIStroke", Dropdown)
            DropCardStroke.Thickness = 1.5
            DropCardStroke.Color     = Color3.fromRGB(45, 45, 45)
            ...
            DropdownContent.Position       = UDim2.new(0, 15, 0, 25)
            DropdownContent.Size           = UDim2.new(1, -180, 0, 12)
            DropdownContent.Name           = "DropdownContent"
            DropdownContent.Parent         = Dropdown

            SelectOptionsFrame.AnchorPoint        = Vector2.new(1, 0.5)
            SelectOptionsFrame.BackgroundColor3   = Color3.fromRGB(40, 40, 40)
            ...
            SelectOptionsFrame.LayoutOrder        = dropdownPageCounter
            SelectOptionsFrame.Parent             = Dropdown

            -- Animated gradient border on the selector pill (always-on glow)
            local SelStroke = Instance.new("UIStroke", SelectOptionsFrame)
            SelStroke.Thickness = 1.4
            ...
            -- Open panel (connected after container declaration)
            local _connected = false
            local function connectDropdownButton(container)
                ...
                DropdownButton.Activated:Connect(function()
                    if not MoreBlur.Visible then
                        ...
                        TweenService:Create(DropdownSelect,
                            ...
                            { Position = UDim2.new(1, -11, 0.5, 0) }):Play()
                        -- Highlight the card border while panel is open
                        TweenService:Create(DropCardStroke, TweenInfo.new(0.2), { Color = GuiConfig.Color, Thickness = 1.5 }):Play()
                    end
                end)
            end
            ...
            DropdownContainer.LayoutOrder        = dropdownPageCounter
            DropdownContainer.Parent             = DropdownFolder

            connectDropdownButton(DropdownContainer)
            ...
            ScrollSelect.Size                    = UDim2.new(1, 0, 1, -36)
            ScrollSelect.Position                = UDim2.new(0, 0, 0, 36)
            ...
            function DropdownFunc:AddOption(option)
                ...
                OptionText.Position         = UDim2.new(0, 10, 0, 8)
                OptionText.Size             = UDim2.new(1, -40, 0, 13)
                ...
                OptionButton.MouseEnter:Connect(function()
                    if Option.BackgroundTransparency > 0.5 then
                        TweenService:Create(Option, TweenInfo.new(0.1), { BackgroundTransparency = 0.85 }):Play()
                    end
                end)
                OptionButton.MouseLeave:Connect(function()
                    local sel = DropdownConfig.Multi and tableFind(DropdownFunc.Value, value) or DropdownFunc.Value == value
                    if not sel then
                ...
                OptionButton.Activated:Connect(function()
                    if DropdownConfig.Multi then
                        if not tableFind(DropdownFunc.Value, value) then
            ...
            function DropdownFunc:Set(Value)
                ...
                local ti1 = TweenInfo.new(0.1)

                for _, Drop in ScrollSelect:GetChildren() do
                    if Drop.Name == "Option" and Drop:FindFirstChild("OptionText") then
                        local v        = Drop:GetAttribute("RealValue")
                        local selected = DropdownConfig.Multi and tableFind(DropdownFunc.Value, v) or DropdownFunc.Value == v

                        if selected then
                            anySelected = true
                            TweenService:Create(Drop.ChooseFrame, ti2, { Size = UDim2.new(0, 2, 0, 14) }):Play()
                            TweenService:Create(Drop.ChooseFrame.UIStroke, ti2, { Transparency = 0 }):Play()
                            ...
                            TweenService:Create(Drop.ChooseFrame, ti1, { Size = UDim2.new(0, 0, 0, 0) }):Play()
                            TweenService:Create(Drop.ChooseFrame.UIStroke, ti1, { Transparency = 0.999 }):Play()
                            TweenService:Create(Drop, ti1, { BackgroundTransparency = 1 }):Play()
                            TweenService:Create(Drop.OptionText, ti1, { TextColor3 = Color3.fromRGB(230, 230, 230) }):Play()
                        end
                    end
                end
                ...
                -- Card border glow when something is selected
                if anySelected then
                    TweenService:Create(DropCardStroke, TweenInfo.new(0.25), { Color = GuiConfig.Color }):Play()
                else
                    TweenService:Create(DropCardStroke, TweenInfo.new(0.25), { Color = Color3.fromRGB(45, 45, 45) }):Play()
                end
            ...
            dropdownPageCounter = dropdownPageCounter + 1
