-- ══════════════════════════════════════════════
--  BlessedHubX  v12  —  Library  [PREMIUM EDITION]
...
--  colorFunc nil = uses accent from window config
--  colorFunc = Color3 based = single hue, brightness pulses only (NO rainbow)
-- ════════════════════════════════════════════
local function MakeAnimatedGradient(stroke, anchorFrame, colorFunc)
    ...
    conn = RunService.Heartbeat:Connect(function(dt)
        ...
        if colorFunc then
            ...
            -- Fallback static white (should never hit this in normal usage)
...
-- Helper: build a single-hue pulsing color func from a Color3
...
function BlessedHubX:Window(GuiConfig)
    ...
    -- AnimColor: ALWAYS a single fixed color — hue never changes, only brightness pulses
    ...
    -- Single-hue pulse color func for animated borders
    ...
    if not isMobile then
        Close.MouseEnter:Connect(function() TweenService:Create(CloseImg,TweenInfo.new(0.15),{ImageColor3=Color3.fromRGB(255,60,60)}):Play() end)
    ...
    -- Accent line under divider (thin color strip)
    ...
    -- Spacer frame so items start below the header
    ...
    -- This will be in ScrolLayers, one per tab
    ...
    local function ToggleWindow()
        if not IsMinimized then
            IsMinimized=true
            TweenService:Create(DropShadowHolder,TweenInfo.new(0.25,Enum.EasingStyle.Quint,Enum.EasingDirection.In),
                {Size=UDim2.new(0,WSIZE.X.Offset,0,0)}):Play()
            task.wait(0.27); DropShadowHolder.Visible=false
        else
            IsMinimized=false
            DropShadowHolder.Size=UDim2.new(0,WSIZE.X.Offset,0,0)
            DropShadowHolder.Visible=true
            TweenService:Create(DropShadowHolder,TweenInfo.new(0.35,Enum.EasingStyle.Back,Enum.EasingDirection.Out),
                {Size=WSIZE}):Play()
        end
    end
    ...
    Close.Activated:Connect(function()
        ...
        overlay.BackgroundTransparency=1; overlay.ZIndex=50; overlay.Parent=Main
        ...
        dialog.AnchorPoint=Vector2.new(0.5,0.5); dialog.ZIndex=51; dialog.Parent=overlay
        ...
        dlgLbl("Close Hub?",true,16,Color3.fromRGB(255,255,255),UDim2.new(0,0,0,0),UDim2.new(1,0,0,48),52)
        dlgLbl("You won't be able to reopen without re-executing.",false,11,Color3.fromRGB(140,140,160),
            UDim2.new(0,14,0,44),UDim2.new(1,-28,0,50),52)
        ...
        TweenService:Create(dialog,TweenInfo.new(0.3,Enum.EasingStyle.Back,Enum.EasingDirection.Out),
            {Size=UDim2.new(0,290,0,148),Position=UDim2.new(0.5,0,0.5,0)}):Play()
    ...
    -- ── Dropdown overlay panel (covers full Main) ─
    ...
    function Tabs:AddTab(TabConfig)
        ...
        -- Header spacer so items start below the fixed NameTab label
        ...
        -- Auto LayoutOrder counter for elements in this tab
        ...
        -- ── Card factory ────────────────────
        --  Creates a styled card frame with auto LayoutOrder
        ...
        -- ── Paragraph ───────────────────────
        -- cfg2.Title   = bold heading (optional)
        -- cfg2.Content = body text
        ...
        function TabItems:AddToggle(cfg2)
            ...
            -- AnimColor stroke on selected: use accentColorFunc
            ...
            function tf:Set(val, skip)
                ...
                if val then
                    ...
                    -- Animated stroke on card when active
        ...
        function TabItems:AddButton(cfg2)
            ...
            -- Outer card (acts as padding frame)
            local card,_ = CreateCard(Color3.fromRGB(10,10,16), 0)
            card.Size=UDim2.new(1,-2,0,H)
            -- Remove the default stroke from outer card
        ...
        function TabItems:AddDropdown(cfg2)
            ...
            -- AnimColor stroke on pill (single fixed hue)
        ...
        function TabItems:AddDiscord(DiscordConfig)
            ...
            -- Premium "Join Server" button (text + icon)
            ...
            -- Shift text to not overlap icon
